...
class Register
{
    ...
    public function verificarUsuarioExistente($usuario)
    {
        ...
        $resultado->bindValue(":email", $usuario, PDO::PARAM_STR);
        $resultado->execute();
    ...
    public function insertarUsuario($name, $lastname, $usuario, $contrasenia, $email)
    {
        ...
        $sql = "INSERT INTO usuarios (nombre, apellidos, password, email) VALUES (:nombre, :lastname, :password, :email)";
        $resultado = $this->conexion->prepare($sql);


        $resultado->execute(array(":nombre" => $name, ":lastname" => $lastname, ":password" => $pass_cifrado, ":email" => $email));
...
try {
    ...
    if ($registro_usuario->verificarUsuarioExistente($usuario_input)) {
        ...
        $registro_usuario->insertarUsuario($name, $lastname, $contrasenia_input, $email_input);
